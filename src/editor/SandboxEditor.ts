...
type ParticleArtPattern = FormationPattern;
  ...
  private updateEffects(dt: number): void {
    ...
    for (const item of this.items) {
      if (item.kind === "blackhole") {
        ...
        item.angle += FORMATION_SPIN * dt;
        const slots = computeFormationSlots(item.x, item.y, item.r, item.angle, item.pattern);
        driveFormation(this.system, item.id, slots, FORMATION_FOLLOW, dt);
      }
    }
